...
-- Feature modules needed for toggle handlers that apply/reset directly
...
local rapid_fire = require("features.rapid_fire") -- Needed for Reset function

local M = {}
local KeybindList = {} -- Stores info for the load message

-- Helper to register and track toggleable feature keybinds
local function AddKeybind(key, keyName, mods, featureDisplayName, featureInternalName, immediateApplyFunc, resetFunc)
    RegisterKeyBind(key, mods, handler.CreateToggleHandler(featureDisplayName, featureInternalName, immediateApplyFunc, resetFunc))
    local modStr = ""
...
function M.RegisterAll()
    ...
    -- Register toggleable cheats with F-keys using the helper
    ...
    AddKeybind(Key.F5, "F5", {}, "Rapid Fire (Character/Plane/Tank)", config.Features.RAPID_FIRE, rapid_fire.Apply, rapid_fire.Reset) -- Use Apply/Reset for Character/Plane props
    -- F6 skipped
    ...
    RegisterKeyBind(config.VK_LBUTTON, {}, function()
        -- Callback runs every time VK_LBUTTON (LMB) is pressed
